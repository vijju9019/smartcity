/**
 * platform-hooks - local shim
 * Provides useQuery, useMutation, useCamera, useLocation, useShare, useMaps, useFilePicker
 */
    ...
    return new Promise(function(resolve) {
      ...
      try {
        ...
        ImagePicker.requestCameraPermissionsAsync().then(function(perm) {
          ...
          ImagePicker.launchCameraAsync({ mediaTypes: 'Images', quality: 0.8 }).then(function(result) {
            if (!result.canceled && result.assets && result.assets[0]) {
              var asset = result.assets[0];
              setPhoto(asset);
              resolve({ uri: asset.uri });
            } else {
    ...
    return new Promise(function(resolve) {
      ...
      try {
        ...
        Location.requestForegroundPermissionsAsync().then(function(perm) {
          if (perm.status !== 'granted') { resolve({ error: 'Location permission denied.' }); return; }
          Location.getCurrentPositionAsync({ accuracy: Location.Accuracy.Balanced }).then(function(loc) {
            resolve({ latitude: loc.coords.latitude, longitude: loc.coords.longitude });
          }).catch(function() { resolve({ latitude: 12.9716, longitude: 77.5946 }); });
        }).catch(function() { resolve({ latitude: 12.9716, longitude: 77.5946 }); });
      } catch(e) {
        resolve({ latitude: 12.9716, longitude: 77.5946 });
      }
  ...
  const share = useCallback(function(options) {
    return new Promise(function(resolve) {
      ...
      try {
        ...
        RNShare.share({ message: options.message || '' })
          .then(function(r) { resolve({ success: true }); })
          .catch(function(e) { resolve({ error: e.message }); });
      } catch(e) {
...
function useMaps(initialRegion) {
  ...
  return { MapView: MapView, Marker: Marker, mapRef: mapRef, region: region, setRegion: setRegion, isAvailable: isAvailable };
}
...
function useFilePicker() {
  ...
  const pickDocument = useCallback(function(options) {
    ...
    try {
      ...
      DocumentPicker.getDocumentAsync({ type: (options && options.type) || ['*/*'], copyToCacheDirectory: true })
        .then(function(result) {
          if (!result.canceled && result.assets && result.assets[0]) {
            setLastFile(result.assets[0]);
          }
          setIsLoading(false);
        })
